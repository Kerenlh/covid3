...
# setwd("C:\\Users\\Administrator\\Dropbox\\mtDNA_tree_Build_17")
# setwd("C:\\Users\\Keren\\Dropbox\\mtDNA_tree_Build_17")
setwd("/Users/keren/Dropbox/mtDNA_tree_Build_17")
# data = read.csv("reg_data.csv",string=F)
# data.base = read.csv("exposure_base_data.csv",string=F)
# data.codon = read.csv("codon_exposure_data.csv",string=F)
# data.nbs = read.csv("exposure_neighbors_data.csv",string=F)
...
tmp = c( "L.neighbor","R.neighbor","base",            
         "codon", "codon.pos",             
         "gene","mat_peptide","stem_loop",       
         "notes","notes2","CG","amino_acid")
codons_table[codons_table$gene==0,"gene"]="none"
codons_table[codons_table$mat_peptide==0,"mat_peptide"]="none"
codons_table[codons_table$stem_loop==0,"stem_loop"]="none"
codons_table[codons_table$amino_acid==0,"amino_acid"]="none"
codons_table[codons_table$codon.pos==0,"codon.pos"]="none"

plcs = which(is.element(codons_table$codon,rownames(codons_to_amino_acids)))
...
# functions: #
##############
aggregate_data = function(curr_data){
  agg_cols = NULL
  count = 0
  if (length(which(colnames(curr_data)=="exposure"))!=0){
    agg_cols = paste(agg_cols,"exposure")
    count = count+1
  }
  if (length(which(colnames(curr_data)=="syn_exposure"))!=0){
    if (count!=0){
      agg_cols = paste(agg_cols,"+ syn_exposure")
    }else{
      agg_cols = paste(agg_cols,"syn_exposure")
    }
    count = count+1
  }
  if (length(which(colnames(curr_data)=="non_syn_exposure"))!=0){
    if (count!=0){
      agg_cols = paste(agg_cols,"+ non_syn_exposure")
    }else{
      agg_cols = paste(agg_cols,"non_syn_exposure")
    }
    count = count+1
  }
  if (length(which(colnames(curr_data)=="y"))!=0){
    if (count!=0){
      agg_cols = paste(agg_cols,"+ y")
    }else{
      agg_cols = paste(agg_cols,"y")
    }
    count = count+1
  }
  if (length(which(colnames(curr_data)=="transitions"))!=0){
    if (count!=0){
      agg_cols = paste(agg_cols,"+ transitions")
    }else{
      agg_cols = paste(agg_cols,"transitions")
    }
    count = count+1
  }
  if (length(which(colnames(curr_data)=="transversions"))!=0){
    if (count!=0){
      agg_cols = paste(agg_cols,"+ transversions")
    }else{
      agg_cols = paste(agg_cols,"transversions")
    }
    count = count+1
  ...
  # agg_cols = as.formula(paste(colnames(curr_data)[which(colnames(curr_data)=="exposure")],"+",
  #                             colnames(curr_data)[which(colnames(curr_data)=="syn_exposure")],"+",
  #                             colnames(curr_data)[which(colnames(curr_data)=="non_syn_exposure")],"+",
  #                             colnames(curr_data)[which(colnames(curr_data)=="y")],"+",
  #                             colnames(curr_data)[which(colnames(curr_data)=="transitions")],"+",
  #                             colnames(curr_data)[which(colnames(curr_data)=="transversions")],
  #                             "~",colnames(curr_data)[c(which(colnames(curr_data)=="site"),
  #                                                       which(colnames(curr_data)=="codon"),
  #                                                       which(colnames(curr_data)=="amino_acid"),
  #                                                       which(colnames(curr_data)=="L.neighbor"),
  #                                                       which(colnames(curr_data)=="R.neighbor"))]))
  # 
  if (length(which(colnames(curr_data)=="codon"))!=0){
    agg_cols = agg_cols + codon
  }
  rel_cols = c(which(colnames(curr_data)=="site"),which(colnames(curr_data)=="R.neighbor"),
               which(colnames(curr_data)=="L.neighbor"),which(colnames(curr_data)=="codon"),
               which(colnames(curr_data)=="amino_acid"))
  rel_data = curr_data[,rel_cols]
  dup_pos = which(duplicate(rel_data)==TRUE)
  curr_data = curr_data[-dup_pos,]
  
  aggregate(c(exposure,syn_exposure)~c(amino_acid,R.neighbor),data=curr_data,FUN=sum)
  
  aggregate(.~amino_acid+R.neighbor, data = curr_data, FUN = sum)
  
  if (is.na(selected_input[,aggregate_col])){
    sites = unique(curr_data$site)
    vals = unique(curr_data[,aggregate_col])
    if (length(sites)!=length(vals)){
      for (i in 1:length(sites)){
        plcs = which(curr_data$site==sites[i])
        if (length(plcs)>1){
          curr_vals = unique(curr_data[plcs,aggregate_col])
          if (length(plcs)!=length(curr_vals)){
            for (j in 1:length(curr_vals)){
              val_plcs = plcs[which(curr_data[plcs,aggregate_col]==curr_vals[j])]
              agg_row = curr_data[val_plcs[1],]
            }
          }
        }
      }
...
prepare_data_rm_cols = function(curr_data){
  curr_data_input = curr_data[,1:(ncol(curr_data)-4)] # remove exposure, syn_exposure, non_syn_exposure and output
...
is_neighbor_same_as_codon = function(curr_data,rm_cols,count){
  ...
  # if (is.na(selected_input[,"regions"])){
  #   rm_cols = c(rm_cols,which(colnames(curr_data)=="regions"))
  # }else{rm_cols = c(rm_cols,which(colnames(curr_data)=="regions2"))}
...
# data:
# input = c("L.neighbor","R.neighbor","base","codon","site","codon.pos",
#           "gene","mat_peptide","stem_loop","notes","notes2","CG",
...
# # input = c("codon.pos","coding","backwords","regions","regions2","tRNA")
...
#           "codon_backwords",
#           "R.neighbor_backwords","L.neighbor",
...
# input_iterate = c("L.neighbor","R.neighbor","base","codon","codon.pos",
#                   "gene","mat_peptide","stem_loop","CG","amino_acid")             
                ...
                for (m in 1:iterate_vals[1,"gene"]){
                  ...
                  #   }
                    ...
                    if (dim(curr_data)[1]!=0){
                      ...
                      # curr_data = aggregate_data(curr_data)
                      # curr_data = curr_data[,-which(colnames(curr_data)=="site")]
...
details = details[1:(count-1),]
colnames(details) = colnames(curr_details)
details = data.frame(details)
file_name = paste0("details.codon.",parallel_num,collapse = "")
# setwd("C:\\Users\\user\\Dropbox\\mtDNA_tree_Build_17\\all_details5")
# setwd("C:\\Users\\Keren\\Dropbox\\mtDNA_tree_Build_17\\all_details5")
setwd("C:\\Users\\Administrator\\Dropbox\\mtDNA_tree_Build_17\\all_details6")
