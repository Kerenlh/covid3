create_details = function(parallel_num){
  ...
  setwd("/Users/keren/Dropbox/EC2_files/details2/")
  reg_hash = hash()
  save(reg_hash,file = "reg_hash2")
  load("reg_hash2")
  ...
  # codons_table = codons_table[-plcs,]
  # save(codons_table,file = "codons_table3")
  load("codons_table3")
  ...
  # parallel_vals = expand.grid(1:66,1:23,1:9,1:14)
  parallel_vals = expand.grid(1:66,1:23,1,1:13)
  colnames(parallel_vals) = c("codon","amino_acid","output","gene")
              ...
              for (w in w){# w_max:iterate_vals[1,"amino_acid"]){
                ...
                for (t in t_max:iterate_vals[1,"R.neighbor"]){
                  ...
                  for (u in u_max:iterate_vals[1,"L.neighbor"]){
                    ...
                    for (m in m){# in 1:iterate_vals[1,"gene"]){
                      ...
                      if (dim(curr_data)[1]!=0){
                        ...
                        if (is.na(dim(curr_data)[1])==FALSE){
                          ...
                          if (is.null(reg_hash[[reg_name2]])){
                            curr_details = process_data(curr_data,selected_input,
                                                        selected_output,curr_details,count,i)
                            print(curr_details)
                            reg_hash[[reg_name2]] = curr_details
                          }else{
                            curr_details = reg_hash[[reg_name2]]
                            curr_details[1:length(selected_input)] = selected_input
                            print("Yay")
                          }
  ...
  if(count>0){
    ...
    file_name = paste0("details.q.",q,"_w.",w,"_i.",i,"_m.",m,"_parallel_num.",parallel_num,collapse = "")
    # setwd("/home/ubuntu/Dropbox/EC2_files/trees")
    setwd("/Users/keren/Dropbox/EC2_files/details2/")
    save(details,file = file_name)
    # save(reg_hash,file = "reg_hash")
  }
}
...
registerDoSNOW(cl)
# codon_vals = 37:66
parallel_num = c(1:1144)
print(Sys.time())
details = clusterApply(cl, parallel_num,create_details)
print(Sys.time())
stopCluster(cl)
