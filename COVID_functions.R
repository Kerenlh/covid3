...
return_only_tree_seqs = function(seqs,tree){
  ...
  relevant_seqs = NULL
  while( (i <= length(sorted_seqs_names$x)) && (j <= length(sorted_tree_names))){
    if (sorted_seqs_names$x[i] != sorted_tree_names[j]){
  ...
  new_seqs = seqs[sorted_seqs_names$ix[relevant_seqs]]
  return(new_seqs)
}
